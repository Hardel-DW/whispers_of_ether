...
public record WellComponent(double maxWell, double currentWell) implements TooltipProvider {
    ...
    public static final Codec<WellComponent> CODEC = RecordCodecBuilder.create(instance -> instance.group(
        ...
        Codec.DOUBLE.fieldOf("current_well").forGetter(WellComponent::currentWell)
    ).apply(instance, WellComponent::new));
    ...
    public void addToTooltip(final Item.TooltipContext context, final Consumer<Component> consumer,
        ...
        consumer.accept(Component.translatable("component.whispers_of_ether.well",
            String.format("%.0f", currentWell), String.format("%.0f", maxWell))
                .withStyle(currentWell > maxWell * 0.5 ? ChatFormatting.GREEN :
                          currentWell > maxWell * 0.2 ? ChatFormatting.YELLOW : ChatFormatting.RED));
    }
}
