...
public class EtherObjectCommand {
    ...
    private static CompletableFuture<Suggestions> suggestTypes(CommandContext<CommandSourceStack> ctx,
        ...
        return SharedSuggestionProvider.suggest(
            SceneObjectType.REGISTRY.keySet().stream().map(Identifier::getPath), builder);
    }
    ...
    private static int create(CommandContext<CommandSourceStack> ctx) {
        ...
        Identifier typeId = typeStr.contains(":") ? Identifier.tryParse(typeStr)
            : Identifier.fromNamespaceAndPath(fr.hardel.whispers_of_ether.WhispersOfEther.MOD_ID, typeStr);
